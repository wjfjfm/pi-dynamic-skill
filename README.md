...
- A `Dynamic skills` section is projected into model context with fixed usage/tree-maintenance instructions, followed by active and pending-eviction lists. Both lists use Pi’s native `formatSkillsForPrompt` output (`name`, `description`, `location`), including its escaping and `disable-model-invocation` behavior. Skill bodies are not expanded, and the projected message is not saved in conversation history.
